...
test('creates a room, starts a round, and admits ten players', async () => { const players = await Promise.all(Array.from({ length: 10 }, (_, i) => client('player-' + i))); const createdPromise = next(players[0], 'room_created'); const startedPromise = next(players[0], 'round_started'); message(players[0], 'create_room', { mode: 'race', name: 'player-0' }); const created = await createdPromise; const started = await startedPromise; assert.equal(started.round.size, 4); for (let i = 1; i < players.length; i++) message(players[i], 'join_room', { code: created.code, name: 'player-' + i }); await new Promise(resolve => setTimeout(resolve, 100)); assert.equal(rooms.get(created.code).players.size, 10); players.forEach(ws => ws.close()); });
test('rejects the eleventh player', async () => { const players = await Promise.all(Array.from({ length: 11 }, (_, i) => client('overflow-' + i))); const createdPromise = next(players[0], 'room_created'); const startedPromise = next(players[0], 'round_started'); message(players[0], 'create_room', { mode: 'classic' }); const created = await createdPromise; await startedPromise; for (let i = 1; i < 10; i++) message(players[i], 'join_room', { code: created.code }); await next(players[0], 'room_state'); message(players[10], 'join_room', { code: created.code }); assert.equal((await next(players[10], 'error')).code, 'ROOM_FULL'); players.forEach(ws => ws.close()); });
