...
test("serves standalone evergreen documents, gameplay SPA routes, and redirects transient game screens", async () => {
  ...
  const contentRoutes = [
    ...
    },
    {
      ...
      description: "Learn how to trace words, score points, and play solo or multiplayer in WordRush.",
      h1: "How to Play WordRush",
      excludedContentScreen: "gameModesScreen",
    },
    {
      path: "/game-modes",
      page: "gameModes",
      title: "WordRush Game Modes — Find Your Next Game",
      description: "Explore WordRush game modes, from solo word challenges to multiplayer games with friends.",
      h1: "WordRush Game Modes",
      excludedContentScreen: "howToPlayScreen",
    },
  ];
  for (const contentRoute of contentRoutes) {
    const response = await fetch(origin + contentRoute.path);
    const body = await response.text();
    const canonical = "https://wordrush.party" + contentRoute.path;
    assert.equal(response.status, 200, contentRoute.path);
    assert.match(body, new RegExp("<title>" + contentRoute.title + "<\\/title>"));
    assert.match(body, new RegExp('<meta name="description" content="' + contentRoute.description));
    ...
    assert.match(body, new RegExp('<meta property="og:title" content="' + contentRoute.title + '"'));
    assert.match(body, new RegExp('<meta property="og:description" content="' + contentRoute.description));
    assert.match(body, new RegExp('<meta name="twitter:title" content="' + contentRoute.title + '"'));
...
test("custom Dirty starts directly", async () => {
...
test("custom Dirty admission and reconnect use the normal room flow", async () => {
  const host = await client("custom-dirty-flow-host");
  const createdPromise = next(host, "room_created");
  const lobbyPromise = next(host, "room_state");
  message(host, "create_room");
  const created = await createdPromise;
  ...
  const guest = await client("custom-dirty-flow-guest");
  const joinedPromise = next(guest, "joined_room");
  message(guest, "join_room", { code: created.code });
  const joined = await joinedPromise;
  assert.equal(room.players.has("custom-dirty-flow-guest"), true);
  assert.equal(room.status, "playing");

  guest.close();
  await new Promise((resolve) => setTimeout(resolve, 10));
  const resumed = await client("custom-dirty-flow-guest");
  ...
  assert.equal(resumedState.mode, "custom");
...
test("CUSTOM_WORDS_REJECTED fires before Dirty start", async () => {
...
test("initial eligible Dirty selection starts directly", async () => {
  const { host, guests, code } = await createRoomWithPlayers([
    "initial-dirty-host",
    "initial-dirty-guest",
  ]);
...
test("queued Dirty Random Rush starts and admits a late player", async () => {
  ...
  const guest = await client("queued-dirty-late-guest");
  const joinedPromise = next(guest, "joined_room");
  message(guest, "join_room", { code: created.code });
  await joinedPromise;
  assert.equal(room.players.has("queued-dirty-late-guest"), true);
...
test("initial Random Rush Dirty starts directly", async () => {
  generationTestHooks.randomMode = () => "dirty";
  const { host, guests, code } = await createRoomWithPlayers([
