(() => {
  ...
  const scoreRound = (detail) => {
    // Solo scores are browser-reported and cannot be verified by the server.
    // The public leaderboard accepts authoritative multiplayer results only.
    ...
    const marker = [guestId, detail.roundId || value].join(":");
